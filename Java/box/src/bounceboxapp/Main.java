...
public class Main extends Application {
    ...
    public static void addShapes() throws FileNotFoundException{
        ...
        while (scan.hasNext()) {
            ...
            Circle c1 = new Circle(scan.nextInt(), scan.nextInt(),scan.nextInt());
            if (scan.hasNextInt()){
            c1.setVelocity(scan.nextInt(), scan.nextInt());
            }
            if (scan.hasNextInt()){
                c1.setColor(scan.nextInt(), scan.nextInt(), scan.nextInt());
            } else {
            ...
            box.addShape(c1);
                ...
                Square s1 = new Square(scan.nextInt(),scan.nextInt(),scan.nextInt());
                s1.setColor(Color.BLACK);
                if (scan.hasNextInt()){
                s1.setVelocity(scan.nextInt(), scan.nextInt());
                
                }
                if (scan.hasNextInt()){
                s1.setColor(scan.nextInt(), scan.nextInt(), scan.nextInt());
                } else {
                s1.setColor(Color.BLACK);
            }
                box.addShape(s1);
